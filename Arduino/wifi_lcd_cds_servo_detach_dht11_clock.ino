...
#define SERVER_NAME "10.10.14.71"
#define SERVER_PORT 5000
#define LOGID "JHY_ARD"
...
#define LED_AC_PIN 10
#define LED_GAS_PIN 11
#define LED_DEHUMI_PIN 12
...
char lcdLine1[17] = "WiFi Module KJK"; // "" 묶으면 널 포함되어서 16+1
...
void setup() {
  ...
  pinMode(GAS_PIN, INPUT);    // 화재 감지 핀을 입력으로 설정 (생략 가능)
  pinMode(BUTTON_PIN, INPUT);    // 버튼 핀을 입력으로 설정 (생략 가능)
  pinMode(LED_AC_PIN, OUTPUT); //D10
  pinMode(LED_GAS_PIN, OUTPUT); //D11 
  pinMode(LED_DEHUMI_PIN, OUTPUT); //D12
...
void loop() {
  ...
  if (timerIsrFlag) // 1초에 한번씩 실행할 내용
  {
    ...
    if (!(secCount % 5)) //5초에 한번씩 실행
    {
      ...
      sprintf(sendBuf, "AC@%s]\n", AC_po ? "ON" : "OFF");
      client.write(sendBuf, strlen(sendBuf));
      client.flush();

      if(DEHUMI_po==false){        
        if(humi>55){
          digitalWrite(LED_DEHUMI_PIN, HIGH);
          DEHUMI_po =true;
        }
        else if(humi <50){
          digitalWrite(LED_DEHUMI_PIN, LOW);
          DEHUMI_po =false;          
        }
      }
      sprintf(sendBuf, "DEHUMI@%s]\n", DEHUMI_po ? "ON" : "OFF");
      client.write(sendBuf, strlen(sendBuf));
      client.flush();
...
void socketEvent()
{
  ...
  else if(!strcmp(pArray[1],"DEHUMI")){
    if (!strcmp(pArray[2], "ON")) {
      DEHUMI_po = true;
      digitalWrite(LED_DEHUMI_PIN, HIGH);
      sprintf(sendBuf, "DEHUMI@%s]\n","ON");

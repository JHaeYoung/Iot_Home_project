...
void updateIoTKitchenTable(char *msg);
...
void * clnt_connection(void *arg)
{
        ...
        while(1)
        {
				...
				updateIoTKitchenTable(msg_copy);
...
void updateIoTKitchenTable(char *msg) {
    char *delimiter_dir = ">";
	char *delimiter_net = "@";
	//char *token_trash = strtok(msg, delimiter_dir);
	//printf("token_trash : %s\n",token_trash);
    char *token_column = strtok(msg, delimiter_net);
	//printf("token_column : %s\n",token_column);
	char *token_value;
    if (token_column != NULL) {
        // 뒤의 데이터를 추출하고 조건에 따라 업데이트 수행
        if (strcmp(token_column, "GAS") == 0 || strcmp(token_column, "LIGHTING") == 0 || strcmp(token_column, "DEHUMI") == 0|| strcmp(token_column, "AC") == 0) {
            token_value = strtok(NULL, delimiter_net); // @ 뒤의 데이터 추출
			
			char *closingBracket = strchr(token_value, ']');
            ...
            if (token_value != NULL && strlen(token_value) > 0) {
                ...
                char query[BUF_SIZE];
                snprintf(query, sizeof(query), "UPDATE iot_kitchen SET %s = '%s' WHERE id = 1;",token_column, token_value);
